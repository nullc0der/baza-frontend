...
const shouldIgnoreCommand = (cmd)=> {
	...
	const cleanedPath = cmd.replace(process.cwd(), '')
...
const run = (cmd = false, log = false)=> {
	if (!cmd || typeof cmd !== 'string')
		return Promise.reject( new Error('Invalid command. Should only be a string.') )

	if ( shouldIgnoreCommand(cmd) )
		return Promise.reject( new Error('run:IGNORE_LIST has similar command ' + cmd) )

	return new Promise((res, rej)=> {
		console.log(
			chalk.cyan(`Running command: `) + chalk.yellow(cmd)
		)
