...
const debug = require('debug')('baza:server/configure')
...
export function readDependencies(app){
	...
	console.log(chalk.cyan('Webpack Assets from: ' + assets_path ));
	...
	debug('Found assets: ', JSON.stringify(assets, null, 2) )
...
export function configureServer(app){
	...
	app.locals.pretty  = process.env.DIST_MODE === "1" || process.env.NODE_ENV !== 'production'
	...
	app.get('/sw.js', CachedFileResponseMiddleware(
		__dirname + '/public/sw.js',
		'text/javascript'
	))
	app.get('/workbox-sw.js', CachedFileResponseMiddleware(
