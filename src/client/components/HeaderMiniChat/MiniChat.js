...
import s from './HeaderMiniChat.scss'
...
class MiniChat extends Component {
  ...
  renderOneChat = (chat, i) => {
    const cx = classnames(
      ...
      'a-center',
      'j-between',
      getOnlineStatus(chat.status)
    )
    return (
      <div key={i} className="mini-chat flex-vertical">
        <div className={cx}>
  ...
  render() {
    ...
    return <div className={cx}>{chats.map(this.renderOneChat)}</div>
  }
}
